...
#Command for calling the script in the command line: R < fig4_bar_chart_fia.r --vanilla
...
#Query for dead trees: 2014 to 2019
...
#Query for live trees 2001 - 2006
...
live.2007 %>% group_by(INVYR) %>% count()

#Query for dead trees 2001 - 2006
...
dead.2007 %>% group_by(INVYR) %>% count()
...
trees.2007$ACTUALHT <- trees.2007$ACTUALHT * (1/3.281) #Convert height in feet to meters.
trees.2007$HT <- trees.2007$HT * (1/3.281) #Convert height in feet to meters.

trees.2017$ACTUALHT <- trees.2017$ACTUALHT * (1/3.281) #Convert height in feet to meters.
trees.2017$HT <- trees.2017$HT * (1/3.281) #Convert height in feet to meters.

trees.2007$live_biomass <- trees.2007$live_biomass * (1/2205) #Convert lbs to Mg

trees.2017$live_biomass <- trees.2017$live_biomass * (1/2205) #Convert lbs to Mg

trees.2007$DIA <- trees.2007$DIA * (2.54) #Convert inches to centimeters. (1 in = 2.54 cm)

trees.2017$DIA <- trees.2017$DIA * (2.54) #Convert inches to centimeters.

trees.2007$count <- trees.2007$count * (1/0.404686) #Convert from trees per acre to trees per hectare

trees.2017$count <- trees.2017$count * (1/0.404686) #Convert from trees per acre to trees per hectare
...
trees.all$basal_area <- (((trees.all$DIA / 2)^2) * pi)*(1/10000) * trees.all$count # convert to m^2 / ha (from cm^2 / ha)
...
trees.all$DIA.group = with(trees.all, factor(DIA.group, levels = c('12.7-29 cm', '30+ cm')))
...
DIA.live.small <- subset(trees.group, Tree_Status == 'Live' & DIA.group == '12.7-29 cm')
...
join.DIA.live.small$DIA.group <- '12.7-29 cm'
join.DIA.live.small$Tree_Status <- 'Live'

DIA.live.mid <- subset(trees.group, Tree_Status == 'Live' & DIA.group == '30+ cm')
...
join.DIA.live.mid$DIA.group <- '30+ cm'
join.DIA.live.mid$Tree_Status <- 'Live'

DIA.live <- rbind(join.DIA.live.small, join.DIA.live.mid) 
rmv <- c('Tree_Status')
DIA.live <- dplyr::select(DIA.live, -rmv)

#Dead plots
DIA.dead.small <- subset(trees.group, Tree_Status == 'Dead' & DIA.group == '12.7-29 cm')
...
join.DIA.dead.small$DIA.group <- '12.7-29 cm'
join.DIA.dead.small$Tree_Status <- 'dead'

DIA.dead.mid <- subset(trees.group, Tree_Status == 'Dead' & DIA.group == '30+ cm')
...
join.DIA.dead.mid$DIA.group <- '30+ cm'
join.DIA.dead.mid$Tree_Status <- 'Dead'

DIA.dead <- rbind(join.DIA.dead.small, join.DIA.dead.mid) #, join.DIA.dead.large)
DIA.dead <- dplyr::select(DIA.dead, -rmv)
DIA.join <- left_join(trees.sum, DIA.live, by = c('PLOT', 'drought', 'DIA.group'))
...
#Query for FIA dead trees, 2014-2019
...
dead.second.2017 #%>% group_by(INVYR) %>% count()
live.second.2017 %>% group_by(INVYR) %>% count()

#Query for FIA live trees 2001 - 2006
...
#Query for FIA dead trees 2001 - 2006
...
#Convert units 
trees.second.2007$ACTUALHT <- trees.second.2007$ACTUALHT * (1/3.281) #Convert height in feet to meters.
trees.second.2007$HT <- trees.second.2007$HT * (1/3.281) #Convert height in feet to meters.

trees.second.2017$ACTUALHT <- trees.second.2017$ACTUALHT * (1/3.281) #Convert height in feet to meters.
trees.second.2017$HT <- trees.second.2017$HT * (1/3.281) #Convert height in feet to meters.

trees.second.2007$live_biomass <- trees.second.2007$live_biomass * (1/2205) #Convert lbs to Mg

trees.second.2017$live_biomass <- trees.second.2017$live_biomass * (1/2205) #Convert lbs to Mg

trees.second.2007$DIA <- trees.second.2007$DIA * (2.54) #Convert inches to centimeters. (1 in = 2.54 cm)

trees.second.2017$DIA <- trees.second.2017$DIA * (2.54) #Convert inches to centimeters.

trees.second.2007$count <- trees.second.2007$count * (1/0.404686) #Convert from trees per acre to trees per hectare

trees.second.2017$count <- trees.second.2017$count * (1/0.404686) #Convert from trees per acre to trees per hectare

# trees.second.2007$STATUSCD <- factor(trees.second.2007$STATUSCD)
...
# trees.second %>% filter(STATUSCD == 1)

trees.second$basal_area <- (((trees.second$DIA / 2)^2) * pi)*(1/10000) * trees.second$count # convert to m^2 / ha (from cm^2 / ha)
...
type.summary.all
#Tree mortality in Basal Area by species type. It would be good to do the mortality analysis with both basal area and % Basal Area
...
type.both.all
#Trying a violin chart instead of a bar chart
type.both.all$tree_type <- factor(type.both.all$tree_type, levels = c("pine/fir", "other tree"))

#Update bar chart to do mortality %
#FIA die-off dotplot
p11_texta <- data.frame(label = c("a", "bc", "b", "c"),
                      sequence   = c('Both Droughts', 'Both Droughts', '2012-2015 Only', '2012-2015 Only'),
                      y     = c(39.5, 16, 8, 22.5),
                      x     = c(1, 2, 1, 2)
)

p11_textb <- data.frame(label = c("n = 44", "n = 26", "n = 199", "n = 192"),
                        sequence   = c('Both Droughts', 'Both Droughts', '2012-2015 Only', '2012-2015 Only'),
                        y     = c(34.5, 11, 3, 17.5),
                        x     = c(1, 2, 1, 2)
)

p11 <- ggbarplot(filter(type.both.all, tree_type == "pine/fir"), x = "drought", y = "basal_area.mort", fill = "sequence", 
       ...
       geom_text(data = p11_texta, mapping = aes(x = x, y = y, label = label), size = 5) +
       geom_text(data = p11_textb, mapping = aes(x = x, y = y, label = label), size = 3) +
...
p12_text <- data.frame(label = c("a", "a", "b", "b"),
                       sequence   = c('Both Droughts', 'Both Droughts', '2012-2015 Only', '2012-2015 Only'),
                       y     = c(19.5, 16.5, 31.5, 32.5),
                       x     = c(1, 2, 1, 2)
)

p12_number <- data.frame(label = c("n = 44", "n = 26", "n = 199", "n = 192"),
                         sequence   = c('Both Droughts', 'Both Droughts', '2012-2015 Only', '2012-2015 Only'),
                         y     = c(16.7, 13.5, 28.7, 29.5),
                         x     = c(1, 2, 1, 2)
)

p12 <-   ggbarplot(filter(type.both.all, tree_type == "pine/fir"), x = "drought", y = "basal_area", fill = "sequence",
  ...
  theme(legend.background = element_rect(colour = NA, fill = NA), legend.justification = c(1, 0), legend.position = c(0.48, 0.55),
        ...
        axis.text.y = element_text(size = 8), axis.title.y = element_text(size = 10)) +
  geom_text(data = p12_text, mapping = aes(x = x, y = y, label = label), size = 5) +
  geom_text(data = p12_number, mapping = aes(x = x, y = y, label = label), size = 3) + 
...
# p12
# f6 <- ggarrange(p11, p12, ncol = 1, nrow = 2, labels = c('a', 'b'), common.legend = FALSE)
# f6
# ggsave(filename = 'Fig6_basal_area_boxplot.png', height=7, width=14, units = 'cm', dpi=900)

#Basal Area mortality barplot figure
p13_text <- data.frame(label = c("a", "b", "b", "a"),
                       sequence   = c('Both Droughts', 'Both Droughts', '2012-2015 Only', '2012-2015 Only'),
                        y     = c(4.5, 2.4, 0.8, 4),
                        x     = c(1, 2, 1, 2) 
)

p13_number <- data.frame(label = c("n = 44",  "n = 26", "n = 199","n = 192"),
                         sequence   = c('Both Droughts', 'Both Droughts', '2012-2015 Only', '2012-2015 Only'),
                        y     = c(4, 1.9, 0.3, 3.5),
                        x     = c(1, 2, 1, 2)
)
# type.both.all
p13 <-   ggbarplot(filter(type.both.all, tree_type == "pine/fir"), x = "drought", y = "basal_area.dead", fill = "sequence",
                  ylab = expression('Mortality (m'^2*' ha'^-1*')'), xlab = "Time Period", order = c("1999-2002", "2012-2015"),
                  add = "mean_se", error.plot = "errorbar", alpha = 0.8, position = position_dodge()) + 
                  #add.params = list(width = 0.25, size = 0.25, color = "sequence"), trim = TRUE, alpha = 0.6, size = 0.25) +
  guides(color = "none", fill = "none") + #ylim(0, 3.8) +
  # scale_fill_grey(start = 0.8, end = 0.3, name = 'Tree Species', aesthetics = "fill", labels = c("pine/fir" =  "Needleleaf \nConifer", "other tree" = "All Others")) +
  # scale_color_grey(start = 0.8, end = 0.3, name = 'Tree Species', aesthetics = "color", labels = c("pine/fir" =  "Needleleaf \nConifer", "other tree" = "All Others")) +
  scale_fill_manual(values = c("#E66100", "#5D3A9B"), name = 'Drought \nSequence', labels = c("Both Droughts" =  "Both \nDroughts", "2012-2015 Only" = "2012-2015 \nOnly"), aesthetics = "fill") +
  # scale_color_manual(values = c("#E66100", "#5D3A9B"), name = 'Drought \nSequence', labels = c("Both Droughts" =  "Both \nDroughts", "Second Drought" = "2012-2015 \nOnly"), aesthetics = "color") +
  #scale_x_discrete(limits = c("pine/fir", "other tree"), labels = c("pine/fir" =  "Needleleaf Conifer", "other tree" = "All Others")) +
  theme_bw() + #facet_grid( ~ sequence) + 
  theme(legend.background = element_rect(colour = NA, fill = NA), legend.justification = c(1, 0), legend.position = "none",
        legend.text = element_text(size = 6), legend.title = element_text(size = 8), legend.direction = "vertical", 
        axis.text.x = element_text(size = 8), axis.title.x = element_text(size = 10), strip.background = element_blank(),
        strip.text.x = element_blank()) +
  geom_text(data = p13_text, mapping = aes(x = x, y = y, label = label), size = 5) + 
  geom_text(data = p13_number, mapping = aes(x = x, y = y, label = label), size = 3)+
  facet_grid(~ factor(sequence, levels = c('Both Droughts', '2012-2015 Only')))
# p13

# f6 <- ggarrange(p12, p13, ncol = 1, nrow = 2, labels = c('a', 'b'), heights = c(1, 1.1))
# f6

# ggsave(filename = 'SFig7_needleleaf_conifer_mortality_boxplot.png', height=14, width=14, units = 'cm', dpi=900)

#Doing the normal analysis
type.aov.dead <- aov(data = filter(type.both.all, tree_type == 'pine/fir'), basal_area.mort ~ drought * sequence)
...
#ANOVA of mortality by tree species type
# type.aov.2 <- aov(data = filter(type.both.all, tree_type == "pine/fir"), basal_area.mort ~ drought*sequence)
# 
# #Tukey Posthoc of mortality by tree species type
# type.tHSD.2 <- TukeyHSD(type.aov.2)
...
#Save summary statistics by tree species
tb7 <- type.summary.all %>% dplyr::select("tree_type", "drought", "sequence", "sample.size", "biomass.mean", "biomass.se", "count.mean", "count.se", "basal_area.mean", "basal_area.se",        
...
as_image(x = tb7, width = 5, file = "STable9_Southern_California_mortality_summary_statistics_v1.png", zoom = 4.0) 
