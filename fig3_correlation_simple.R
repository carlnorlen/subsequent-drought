...
# install.packages(c("hexbin"),repo='https://cloud.r-project.org/')

#Install packages

# install.packages(c('mgcv'),repo='https://cloud.r-project.org/')
...
#Command for calling the script in the command line: R < fig3_correlation.r --vanilla
#Read in csv data for LANDFIRE

#Improvement possibilty: Could all of the file loading and joining be accomplished by for loops instead?
#Regression Data Sets
# dir_in <- "C:\\Users\\Carl\\mystuff\\Large_Files\\Landsat"
...
#Additional columns of residuals and predicted values for the whole region regression data
...
all.ca <- all.ca %>% mutate(ADS_2004.cat = case_when(
                           (ADS_2004 / 2.41705) >= 2 ~ 1, 
                           (ADS_2004 / 2.41705) < 2 ~ 0),
                           ADS_2017.cat = case_when(
                           (ADS_2017 / 2.41705) >= 2 ~ 1, 
                           (ADS_2017 / 2.41705) < 2 ~ 0))
all.ca$ADS_2004
unique(all.ca$ADS_2017)
summary(all.ca)
...
#Plot dieback (dNDMI) with SPI48 grid
