...
# trees.group <- subset(trees.all, tree_type == 'pine/fir') %>% 
#                group_by(DIA.group, PLOT, Tree_Status, drought) %>% summarize(
#   biomass = sum(live_biomass),
#   count = sum(count),
#   basal_area = sum(basal_area)
# )
...
# DIA.sum <- subset(trees.all, tree_type == 'pine/fir') %>%  
#            group_by(DIA.group, PLOT, drought) %>% summarize(
#   biomass = sum(live_biomass),
#   count = sum(count),
#   basal_area = sum(basal_area)
# )
...
# plot.group <- subset(trees.all, COMMON_NAME == 'ponderosa pine') %>% #changed the subset to just ponderosa pine, tree_type == 'pine/fir') %>%  
#   group_by(PLOT, Tree_Status, drought) %>% summarize(
#   biomass = sum(live_biomass),
#   count = sum(count),
#   basal_area = sum(basal_area)
# )
...
# 
# #Create tree diameter and plot group
# trees.DIA <- subset(trees.all, tree_type == 'pine/fir') %>% 
#   group_by(DIA.group, PLOT, Tree_Status, drought) %>% summarize(
#   biomass = sum(live_biomass),
#   count = sum(count),
#   basal_area = sum(basal_area)
...
#' 
#' Species.All.Death.Summary <- trees.all %>% filter(drought == '1999-2002') %>% group_by(COMMON_NAME, Tree_Status, PLOT) %>% 
#'                              summarize(DIA.mean = sum(DIA*count)/sum(count), trees = sum(count), count = n())
#' 
#' Species.Second.Death.Summary <- trees.second %>% filter(drought == '2012-2015') %>% group_by(COMMON_NAME, Tree_Status, PLOT) %>% 
#'                                 summarize(DIA.mean = sum(DIA*count)/sum(count), trees = sum(count), count = n())
#' 
...
#' 
#' #Get the totals by plot by DIA group and drougth
#' DIA.second.sum <- subset(trees.second, tree_type == 'pine/fir') %>% 
#'   group_by(PLOT, drought, DIA.group) %>% summarize(
#'   biomass = sum(live_biomass),
#'   count = sum(count),
#'   basal_area = sum(basal_area)
#' )
...
#' 
#' #Create tree diameter and plot group
#' trees.second.DIA <- subset(trees.second, tree_type == 'pine/fir') %>% 
#'   group_by(DIA.group, PLOT, Tree_Status, drought) %>% summarize(
#'   biomass = sum(live_biomass),
#'   count = sum(count),
#'   basal_area = sum(basal_area)
...
#' 
#' #Get the data for the full plot
#' plot.second.group <- subset(trees.second, tree_type == 'pine/fir') %>%  
#'                      group_by(PLOT, Tree_Status, drought) %>% summarize(
#' 									   biomass = sum(live_biomass),
#' 									   count = sum(count),
#' 									   basal_area = sum(basal_area)
...
#' DIA.aov.dead <- aov(data = filter(DIA.both.all,  sequence == '2012-2015 Only' & drought == '2012-2015'), 
#'                     basal_area.mort ~ DIA.group)
...
#Redoing the data so that I don't have to duplicate the code
#Calculating the mortality rate by species group 
...
#Bar chart by tree species
  p1 <- ggbarplot(filter(type.combine.all), x = "drought", y = "basal_area.mort", fill = "COMMON_NAME", color = 'sequence', 
                  ylab = 'Mortality (%)', xlab = "Time Period", order = c("1999-2002", "2012-2015"), position = position_dodge(), #stat = "density",
                  add = "mean_se" , error.plot = "errorbar", alpha = 0.8) + 
    guides(color = "none", fill = guide_legend('Tree \nType')) + theme_bw() +
    scale_color_manual(values = c("black", "black"), name = 'Drought \nSequence', 
